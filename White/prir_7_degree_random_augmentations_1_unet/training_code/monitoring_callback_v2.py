"""
Monitoring Callback for View-Conditioned Model (V2 - Arbitrary Views)
=======================================================================
...
class MonitoringCallback(Callback):
    """Callback to save sample predictions and per-view metrics (supports arbitrary views)"""
    ...
    def __init__(self, save_dir, num_samples=5, save_every_n_epochs=5):
        ...
        # Track per-view metrics (dynamically populated)
        self.per_view_metrics = {}
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        if self.per_view_metrics:
            print(f"\n=== Epoch {epoch} Per-View Metrics ===")
            
            for view_name in sorted(self.per_view_metrics.keys()):
        ...
        # Reset for next epoch (keep existing keys)
        for key in self.per_view_metrics:
            self.per_view_metrics[key] = []
    ...
    def _accumulate_metrics(self, outputs):
        """Accumulate per-view accuracy metrics (dynamically track view names)"""
        labels = outputs['labels'].cpu()
        preds = outputs['preds'].cpu()
        view_names = outputs['view_names']
        ...
        for i in range(batch_size):
            ...
            # Dynamically add view_name if not seen before
            if view_name not in self.per_view_metrics:
                self.per_view_metrics[view_name] = []
...
if __name__ == '__main__':
    ...
    print("Supports arbitrary views with dynamic tracking")
