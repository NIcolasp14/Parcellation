...
VIEW_NAMES = [
    'Front', 'Back', 'Left', 'Right', 'Top', 'Bottom',  # 0-5: Cardinal views
    'FrontLeft', 'FrontRight', 'BackLeft', 'BackRight',  # 6-9: Corner views
    'TopFront', 'TopBack', 'TopLeft', 'TopRight',        # 10-13: Top diagonals
    'BottomFront', 'BottomBack', 'BottomLeft', 'BottomRight',  # 14-17: Bottom diagonals
    'LeftTop', 'RightTop'  # 18-19: Side-top views
]
VIEW_ROTATIONS = {
    # Cardinal views (0-5)
    ...
    # Corner views (6-9)
    'FrontLeft':  (0.0, -np.pi/4, np.pi),  # View 6
    'FrontRight': (0.0, np.pi/4, np.pi),   # View 7
    'BackLeft':   (np.pi, np.pi/4, 0.0),   # View 8
    'BackRight':  (np.pi, -np.pi/4, 0.0),  # View 9
    
    # Top diagonal views (10-13)
    'TopFront':  (-np.pi/4, 0.0, np.pi),   # View 10
    'TopBack':   (-3*np.pi/4, 0.0, 0.0),   # View 11
    'TopLeft':   (-np.pi/4, -np.pi/2, 0.0), # View 12
    'TopRight':  (-np.pi/4, np.pi/2, 0.0),  # View 13
    
    # Bottom diagonal views (14-17)
    'BottomFront': (np.pi/4, 0.0, np.pi),   # View 14
    'BottomBack':  (3*np.pi/4, 0.0, 0.0),   # View 15
    'BottomLeft':  (np.pi/4, -np.pi/2, 0.0), # View 16
    'BottomRight': (np.pi/4, np.pi/2, 0.0),  # View 17
    
    # Side-top views (18-19)
    'LeftTop':  (-np.pi/4, -np.pi/2, 0.0), # View 18
    'RightTop': (-np.pi/4, np.pi/2, 0.0),  # View 19
}
...
class ViewConditionedDataset(Dataset):
    ...
    def __init__(self, data_dir, split='train', 
        ...
        print(f"  View encoding: One-hot (20 discrete views) - robust to camera variations")
    ...
    def __getitem__(self, idx):
        ...
        # View one-hot: (20,)
        view_onehot = torch.zeros(20, dtype=torch.float32)
    ...
    def _render_view(self, mesh, labels, view_name, aug_range=7):
        ...
        # Get base rotation for this view from VIEW_ROTATIONS
        base_rx, base_ry, base_rz = VIEW_ROTATIONS[view_name]
        
        # Add random augmentation
        aug_rad = np.deg2rad(aug_range)
        rx = base_rx + np.random.uniform(-aug_rad, aug_rad)
        ry = base_ry + np.random.uniform(-aug_rad, aug_rad)
        rz = base_rz + np.random.uniform(-aug_rad, aug_rad)
        ...
        extmat_transformed = extmat_transformed.reshape(1, 4, 4)  # Shape: (1, 4, 4)
