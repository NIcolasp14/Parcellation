...
def generate_predictions_v2(mesh_path, model, device='cuda:0'):
    ...
    print(f"  Rendering 6 canonical views with PyTorch3D...")
    ...
    for view_idx, view_name in enumerate(pytorch3d_view_order):
        ...
        # NOTE: output_map is already normalized to [0,1], no further normalization needed
        image = image.unsqueeze(0).to(device)  # (1, 3, H, W)
        
        # Create one-hot view encoding
        view_onehot = torch.zeros(6, dtype=torch.float32, device=device)
        view_onehot[our_view_idx] = 1.0
        view_onehot = view_onehot.unsqueeze(0)  # (1, 6)
        ...
        print(f"    ✓ {view_name} (view_id={our_view_idx}): shape {probs_np.shape}")
