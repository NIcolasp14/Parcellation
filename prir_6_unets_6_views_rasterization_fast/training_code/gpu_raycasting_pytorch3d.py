...
def generate_maps_gpu(mesh, labels, intmat, extmat, img_width, img_height, rotation_matrices, recompute_normals=True):
    ...
    # CRITICAL FIX: Brain meshes have ~250K faces - need large bins to avoid overflow!
    # bin_size=None (auto) caused overflow → incomplete renderings
    # bin_size=0 (naive) is accurate but TOO SLOW (30s/batch instead of 2s/batch!)
    # Solution: Use large manual bin_size with high face limits for SPEED + ACCURACY
    raster_settings = RasterizationSettings(
        image_size=(img_height, img_width),
        blur_radius=0.0,
        faces_per_pixel=1,
        bin_size=64,  # Large bins for brain meshes (prevents overflow, maintains speed)
        max_faces_per_bin=300000,  # Very high limit for dense brain meshes (~250K faces)
